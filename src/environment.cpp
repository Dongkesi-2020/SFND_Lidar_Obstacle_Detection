...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer) {
  ...
  std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr,
      ...
      segmentCloud = pointProcessor->SegmentPlane(cloud, 100, 0.2);
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloudClusters =
      pointProcessor->Clustering(segmentCloud.first, 1, 3, 30);
...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer,
  ...
  // ProcessPointClouds<pcl::PointXYZI>* pointProcessorI = new
  // ProcessPointClouds<pcl::PointXYZI>(); pcl::PointCloud<pcl::PointXYZI>::Ptr
  // inputCloud =
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud =
      pointProcessorI->FilterCloud(inputCloud, 0.2,
                                   Eigen::Vector4f(-12, -5, -2, 1),
                                   Eigen::Vector4f(22, 6.5, 2, 1));
  // renderPointCloud(viewer,filterCloud,"filterCloud");

  // Segment
  std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr,
            pcl::PointCloud<pcl::PointXYZI>::Ptr>
      segmentCloud = pointProcessorI->SegmentPlane(filterCloud, 100, 0.2);
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters =
      pointProcessorI->Clustering(segmentCloud.first, 0.4, 10, 1000);

  int clusterId = 0;
  std::vector<Color> colors = {Color(1, 1, 0),     Color(0, 1, 1),
                               Color(1, 0, 1),     Color(0, 0, 1),
                               Color(0, 0.5, 0.5), Color(0.2, 0.5, 1)};

  for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : cloudClusters) {
    std::cout << "cluster size ";
    pointProcessorI->numPoints(cluster);
    // renderPointCloud(viewer,
    // cluster,"obstCloud"+std::to_string(clusterId),colors[clusterId%6]);
