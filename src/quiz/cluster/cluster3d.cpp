...
void proximity(std::vector<bool>& processedPointsIdx, std::vector<int>& cluster,
               int idx, const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  if (processedPointsIdx[idx]) {
    ...
    for (auto i : nearbyPoints) {
      proximity(processedPointsIdx, cluster, i, cloud, tree, distanceTol);
    }
  }
}

std::vector<std::vector<int>> euclideanCluster(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, KdTree* tree,
    float distanceTol) {
...
int main() {
  ...
  for (std::vector<int> cluster : clusters) {
    ...
    for (int indice : cluster)
      clusterCloud->points.push_back(pcl::PointXYZ(cloud->points[indice]));
